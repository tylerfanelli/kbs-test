...
pub async fn resource(_req: HttpRequest, resource_id: web::Path<String>) -> HttpResponse {
    ...
    let plaintext = String::from("hello, SVSM!");

    let mut enc = [0u8; 16];
    enc[..plaintext.len()].copy_from_slice(plaintext.as_bytes());

    aes.encrypt_block((&mut enc).into());

    let encoded = BASE64_STANDARD.encode(enc);
    ...
    let resp = Response {
        protected,
        encrypted_key: private.public_key().to_jwk_string(),
        aad: None,
        iv: "".to_string(),
        ciphertext: encoded,
        tag: "".to_string(),
